  ...
  return (
    <div className={cn("w-full min-w-0", className)}>
      ...
      <div
        ...
        className="relative flex w-full items-stretch gap-0 overflow-hidden rounded-[7px] bg-ink/[0.06] p-px dark:bg-white/[0.08]"
        onKeyDown={(event) => {
        ...
        {options.map((option, index) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative z-0 flex min-h-7 min-w-0 flex-1 items-center justify-center rounded-[7px] px-1.5 font-body text-[11.5px] font-semibold tabular-nums tracking-[-0.01em] touch-manipulation [-webkit-tap-highlight-color:transparent]",
            ...
            >
              {selected ? (
                <motion.span
                  layoutId={thumbId}
                  className="absolute inset-0 -z-10 rounded-[7px] bg-teal shadow-[0_1px_1px_rgb(var(--ink-rgb)_/_0.08),0_1px_2px_rgb(var(--teal-rgb)_/_0.18)] dark:shadow-[0_1px_2px_rgb(0_0_0_/_0.3)]"
                  transition={
                    reduceMotion
                      ? { duration: 0 }
                      : {
