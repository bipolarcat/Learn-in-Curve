...
import { PFQ_SLUG } from "@/lib/courses/registry-data";

/**
 * Every course a user can open, from three independent sources:
 *
 ...
 *  3. Live courses with a signed-in Starter slice (PFQ). Starter is the ABSENCE
 *     of a `feature_entitlements` row, so without this branch a free learner
 *     never sees the course on the dashboard after a Pro grant is removed.
 *
 ...
 * never appeared on the dashboard: it is not fully free, and its paid row lives
 * in `feature_entitlements`. Do not point this back at `entitlements`.
 */
export async function getUserCourses(
  userId: string,
  supabase: SupabaseClient,
): Promise<UserCourse[]> {
  const [{ data: entitlements }, { data: freeCourses }, { data: starterSliceCourses }] =
    await Promise.all([
      supabase
  ...
  for (const course of (starterSliceCourses ?? []) as Course[]) {
    if (!courseMap.has(course.id)) {
      courseMap.set(course.id, {
        ...course,
        access_type: "free",
