...
- **Monetization:** one-time unlocks, never subscriptions. Canonical prices live in `src/lib/courses/registry-data.ts` (`COURSE_STATIC[*].priceCents`) — that value is what Stripe charges and what the pricing cards render, so never quote a price anywhere without deriving it from there. As of 2026-09-21: **PMQ Pro Bundle £20** (Starter free), **PMQ AI Pro Bundle £30** (waitlist, indicative, not charged), **PFQ Pro Bundle £10** (Free scaffolding + LO1 insights; Pro unlocks rest), **PFQ AI Pro Bundle £20** (waitlist, indicative; £10 Pro→AI Pro upgrade later). The £5 Sly fair-usage credit (`SLY_UNLOCK_CREDIT_GBP_CENTS`) rides on the tier that actually carries Sly. Price history: PMQ Pro £9.99 decided 2026-07-01, locked 2026-07-13/14, moved to £8 on 2026-07-31, to £15 on 2026-09-21, **then to £20 the same day**; PMQ AI Pro £15 → £25 → **£30, both on 2026-09-21**; PFQ AI Pro £15 → £20 on 2026-09-15.
...
- **2026-09-22** — Production deploy v3.0 via `npm run deploy -- --set 3.0` (855 commits from wip-2026-08-19 → master). Footer live as Site version 3.0. All 14 branch migrations already present in prod Supabase (dbjoimidfbftammchnql); none applied this session.
- **2026-09-22** — Inbox letter modal: viewport-capped scroll + sticky header/X (iOS); notifications dropdown gets close X (`DashboardInboxBell`).
- **2026-09-22** — Inbox mobile: fixed-position portal dropdown (no shrink-wrapper width bug); letter sheet scrollable via svh + min-h-0; v3 copy refreshed (`DashboardInboxBell`, `dashboard-inbox.ts`).
- **2026-09-22** — Inbox polish: fox logo on letter title; borderless bell; dropdown uses header Menu framer motion; letter modal matches FeedbackModal open animation (`DashboardInboxBell`).
...
- **2026-09-22** — Mock Me hub: “← Back to home” soft-nav (same as Courses catalogue) (`mock-me/page.tsx`, `SoftNavBackLink`).
- **2026-09-22** — Home hero CTAs: “Take free mock exams” → `/mock-me`; “Explore Courses” uses soft-nav ellipsis pending like the mock CTA (`HomeBrandHero`, `ExploreCoursesLink`).
- **2026-09-21** — Pro lock tip: keep inside Learn card (left-align + card clamp); hug copy with `width: max-content` + nowrap (`ProLockHintHost`).
- **2026-09-21** — Locked Insights chip: drop Pro badge beside label on PMQ + PFQ (muted “Insights” only; Pro badge stays on the unlock tip) (`InsightsLockedChip`).
- **2026-09-21** — Pro lock tip: dismiss X; Insights chip `w-fit self-start` so tip anchors to the chip (not mid-column) (`ProLockHintHost`, `InsightsLockedChip`).
- **2026-09-21** — Pro lock tip: anchored under/above the tapped Insights or recall icon (learning-nav placement), Pro badge row-aligned (`ProLockHintHost`).
- **2026-09-21** — PFQ Starter Pro-lock toasts match PMQ: Insights chip (`InsightsLockedChip` / `showProLockHint`); locked mock rows tap → unlock tip (`PfqMockConsole`). PFQ has no Pair/Group/Line content yet.
- **2026-09-21** — Locked Insights / Pair-Group-Line tap toast: “Unlock … with the Pro bundle.” with Pro badge (`showProLockHint`, `InsightsLockedChip`, `ActivityLauncher`).
...
- **2026-09-21** — Shelf article byline: LIC fox logo beside “Learn in Curve” (`LibraryAuthorByline`).
- **2026-09-21** — Shelf list view mobile: date + category share one row under the title (`LibraryHub`).
- **2026-09-21** — Shelf mobile category chips: Apple-style ink fill + paper text when selected (drop orange outline wash) (`LibraryHub`).
- **2026-09-21** — Shelf Grid/List mobile fix v2: CSS-module track (fixed 2.65rem height), translate3d thumb, no Framer — iOS was collapsing the Tailwind/layoutId control (`LibraryViewToggle`).
- **2026-09-21** — Shelf list view: text table (Articles / Date / Category), no illustrations (`LibraryHub`).
- **2026-09-21** — Shelf Grid/List toggle iOS fix: layoutId thumb (not % `x`), 1px border, icon-only under `sm`, text-size-adjust lock (`LibraryViewToggle`).
- **2026-09-21** — Shelf Exam Guides filter icon: GraduationCap (was BookOpen) (`LibraryHub`).
- **2026-09-21** — Shelf sidebar: drop “Filter and sort”, Category always open with icons + article counts; “All Articles” + search placeholder “Search Articles...” (`LibraryHub`).
- **2026-09-21** — Shelf toolbar polish: search focus = border only (no glow); Grid/List outline hairline grey (`LibraryHub`, `LibraryViewToggle`).
- **2026-09-21** — Shelf Grid/List toggle border: ink/25 grey (was reading white at ink/12) (`LibraryViewToggle`).
- **2026-09-21** — Shelf Grid/List toggle polish: cream track + paper thumb, matched 8/5px radii (avoid LIC `rounded-md` = 16px), height locked to search (`LibraryViewToggle`).
- **2026-09-21** — Shelf Grid/List toggle: opaque paper fill (hides site dots), less-round corners, right-aligned in toolbar (`LibraryViewToggle`, `LibraryHub`).
- **2026-09-21** — Shelf layout restage to Claude editorial format: main search + labeled Grid/List toggle, flat Category accordion sidebar, coloured art plates + Fraunces titles + date/tag cards (`LibraryHub`, `LibraryViewToggle`, `libraryIllustrations`).
- **2026-09-21** — Shelf hero lead: first sentence only (`LibraryHub`).
- **2026-09-21** — Shelf hero: add lead under “The Shelf” (`LibraryHub`).
- **2026-09-21** — Shelf H1 centered (`LibraryHub.module.css`).
- **2026-09-21** — Shelf hero: single Fraunces H1 “The Shelf” (Mock Me weight/size); drop “Guides” (`LibraryHub`).
- **2026-09-21** — Shelf: grid/list view toggle with spring thumb (21st view-mode + segmented motion) (`LibraryViewToggle`, `LibraryHub`).
...
- **2026-09-21** — Shelf topic label: exam-prep → “Exam Guides” (`LIBRARY_GROUP_LABELS`).
- **2026-09-21** — Shelf main: remove “N guides” result count (`LibraryHub`).
- **2026-09-21** — Shelf sidebar: remove “Ready to test?” mock CTA (`LibraryHub`).
- **2026-09-21** — Shelf hero: drop lede + published/draft chips (`LibraryHub`).
- **2026-09-21** — Library articles: breadcrumb → “Back to the shelf” (`SoftNavBackLink` in `LibraryArticle`).
...
- **2026-09-21** — Free mock results CTAs: sentence case (drop stamp `uppercase`) (`FreeMockExamClient`).
- **2026-09-21** — Free mock hierarchy: reverted (quiz + FAQ back to matching paper consoles).
- **2026-09-21** — Free mock in-quiz eyebrow: “{mark} mock exam” (was “Free {mark} readiness check”) (`FreeMockExamClient`).
- **2026-09-21** — Header Sign in outline darker (`border-ink/35`, hover `/50`) in `headerPillAuthUtility` (`header-control.ts`).
- **2026-09-21** — Mock Me Start mock: Framer Motion width expand (circle→pill) instead of choppy clip-path (`motion-button.tsx`).
- **2026-09-21** — Mock Me Start mock: circle→pill via `clip-path` (was scaleX fill that flashed square on press) (`motion-button.module.css`).
- **2026-09-21** — Free PMQ mock hero: drop “Complete it within 10 minutes.” (`config.ts`).
- **2026-09-21** — Free PMP mock H1: “Free PMI PMP mock exam” (was “Free PMP readiness check”) (`FreeMockExamShell`).
...
- **2026-09-21** — Mock Me cards: unify card/meta cream with illustration plate (`#f3e8d6`) so art and footer share one ground (`MockMePage.module.css`).
- **2026-09-21** — Mock Me cards: use Sim-supplied character-only art in `public/brand/mock-me/` (pmq/pfq/pmp.png) (`mock-me/page.tsx`).
- **2026-09-21** — Mock Me cards: revert text-free crops; restore homepage syllabus `3.png` / `1.png` / `6.png` (`mock-me/page.tsx`).
...
- **2026-09-21** — Mock Me Start: 21st motion-button (orange circle expands) on click/press only via `MotionButton` + `MockMeExamCard`; no hover (`motion-button.tsx`).
- **2026-09-21** — Mock Me cards: question count sits on the title row (right-aligned) (`mock-me/page.tsx`).
- **2026-09-21** — Mock Me Start mock: drop hover wipe (was flashing while scrolling); press-only scale/nudge (`MockMePage.module.css`).
- **2026-09-21** — Mock Me hub polish: quiet card hover (no lift), Start mock orange wipe + arrow motion, staggered page enter, restore “Looking for a full course? Browse courses” (`mock-me`).
- **2026-09-21** — Mock Me hub cards: unique ink “Start mock” pill + orange arrow disc (not plain text CTA) (`MockMePage.module.css`).
- **2026-09-21** — Mock Me hub lead: “see where you stand” / score / topics / No sign-up. No payment. (`mock-me/page.tsx`).
- **2026-09-21** — Mock Me hub cards: titles APM - PMQ / APM - PFQ / PMI - PMP (`mock-me/page.tsx`).
- **2026-09-21** — Mock Me hub: drop “Free · No account” eyebrow; lead → scoring / revise-topics / no sign-up copy (`mock-me/page.tsx`).
- **2026-09-21** — What's New release registry: `src/content/whats-new.ts` drives nav New chips, dismissible dashboard banner (`WhatsNewBanner`), and public `/whats-new`. Seen-state moves from localStorage (`lic_menu_new_v1`) to `profiles.whats_new_seen_at` (migration `20260921180000_whats_new_seen.sql` **written, unapplied** — Sim applies). Helpers in `src/lib/whats-new/`; `markWhatsNewSeen` server action; PostHog events `whats_new_*`. Badges nav-only; signed-out visitors get no chips. No email wiring this pass.
- **2026-09-21** — **PMQ repriced again, same day, decided by Sim.** PMQ Pro Bundle **£15 → £20**; PMQ AI Pro Bundle **£25 → £30** (still `status: "waitlist"`, indicative, no checkout). PFQ unaffected (Pro £10, AI Pro £20). `COURSE_STATIC["pmq-in-5-days"].priceCents` 1500 → 2000 in `registry-data.ts`; `ai_pro.priceCents` 2500 → 3000 in `src/lib/pmq/plans.ts`. Migration `20260921170000_pmq_pro_price_2000.sql` written **and applied** to `learn-in-curve`: `exam_config.ai_tutor_price_cents` verified at 2000. Tests updated (`course-registry-terms`, `posthog-purchase`, `guest-sly-access`). Docs swept. Note PMQ Pro £20 now equals PFQ AI Pro £20, so a bare `£20` grep is no longer course-unambiguous.
- **2026-09-21** — Mock Me hub: three illustrated picker cards (unused course/feature art), minimal copy; click opens the mock (`mock-me/page.tsx` + CSS). Inspired by Mobbin Duolingo/Babbel selection grids + 21st product-card layout.
- **2026-09-21** — Auth signup course title: sentence case (no uppercase) on PMQ/PFQ preview (`AuthDeskPanel.module.css`).
- **2026-09-21** — Courses catalogue cards: tighten footer bottom padding under “View plans” (`CoursesCatalog.module.css`).
- **2026-09-21** — Preview signup: drop mobile `-3rem` top pull so fox ears clear the fixed header (`PreviewPage.module.css`).
- **2026-09-21** — Revert signup back-button experiment (add + remove); PMQ/PFQ preview cards unchanged (course h2 + prepare lead as `<p>`).
- **2026-09-21** — Remove soft-nav back from signup/preview pages (`preview`, `/auth/sign-up`).
- **2026-09-21** — Signup/preview soft-nav back: label from `?from=` (home/courses/plans/overview); PFQ pricing uses `pfq_pricing` (`soft-nav-back`, preview pages, `/auth/sign-up`).
- **2026-09-21** — PMQ preview signup card matches PFQ: “PMQ in 5 Days” h2 + prepare-for-exam lead as `<p>` (`preview/page.tsx`).
- **2026-09-21** — PFQ preview: course name h2, long signup line as `<p>` (`AuthDeskPanel`, preview page).
- **2026-09-21** — PFQ preview h2: “Create your free account and start preparing for the PFQ exam today.” (`preview/page.tsx`).
- **2026-09-21** — PFQ preview signup card: in-card “PFQ in 2 Days” h1, “Create free account” h2, drop lead (`AuthDeskPanel`, preview page).
- **2026-09-21** — Courses catalogue: PFQ/PMQ “Start free course” use `PfqStartLink` / `PmqStartLink` (guest → preview signup); PFQ preview matches PMQ desk (next → course overview) (`CoursesCatalog`, `pfq/.../preview`).
- **2026-09-21** — PFQ Pro + AI Pro: “565 total practice questions” (was +515) (`plans.ts`).
- **2026-09-21** — PFQ AI Pro: “565 total practice questions” (was +515) (`plans.ts`).
- **2026-09-21** — PMQ Pro/AI Pro: practice + mocks as totals (1,200 / 3 · 1,860 / 4), not increments; Stripe checkout + `pro-included` + `plan-counts` test aligned (`plans.ts`, `actions.ts`).
- **2026-09-21** — PMQ/PFQ pricing H1: smaller clamp (`1.5rem`–`2.15rem`) for “Start free. / Upgrade only if…”.
- **2026-09-21** — PFQ Pro: “Recommended” badge (same teal chip as PMQ) (`plans.ts`, `PfqPlanCards`).
- **2026-09-21** — PMQ Starter: remove badge; Pro badge → “Recommended” (`plans.ts`).
- **2026-09-21** — PMQ AI Pro: show waitlist price again (£30) (`PmqPlanCards` — removed priceRowHidden).
- **2026-09-21** — PMQ Pro badge: “Launch Price” (`plans.ts`).
- **2026-09-21** — PMQ Pro badge: “Introductory Offer” (was “Best Value”) (`plans.ts`).
- **2026-09-21** — PMQ AI Pro: hide waitlist price visually but keep price-row height so card dividers stay aligned (`PmqPlanCards` + CSS).
- **2026-09-21** — PMQ AI Pro: move 60+ recall line under mock exams (`plans.ts`).
- **2026-09-21** — PMQ Pro: move 60+ recall line under mock exams (`plans.ts`).
- **2026-09-21** — PMQ Starter badge: “For Limited Time” (was “Introductory Offer”) (`plans.ts`).
- **2026-09-21** — PMQ AI Pro Sly line: “Sly, the AI tutor” to match PFQ (`plans.ts`).
- **2026-09-21** — PMQ Pro/AI Pro recall row: new `IconRecall` (lucide Shapes) — was reusing practice pad (`PmqPreviewFeatureIcons`, `plans.ts`, `PmqPlanCards`).
- **2026-09-21** — PMQ AI Pro: add Insights + 60+ recall lines under Video & Audio (`plans.ts`), matching Pro.
- **2026-09-21** — PMQ Pro/AI Pro: combine video + audio into one “Video & Audio overview…” line (`plans.ts`).
- **2026-09-21** — PMQ plan cards: remove taglines under the price (`PmqPlanCards`).
- **2026-09-21** — PMQ Pro: add “60+ recall activities…” line under Insights (`plans.ts`).
- **2026-09-21** — PMQ Pro: add Insights line under audio overview (`plans.ts`, `PmqPlanCards`).
- **2026-09-21** — PMQ Starter: add exam structure / mark weighting line under mock exam (`plans.ts`, `PmqPlanCards`).
- **2026-09-21** — PMQ Starter: add takeaways/glossary/misconceptions/memory line under study material; drop separate misconceptions + memory bullets (`plans.ts`).
- **2026-09-21** — PMQ Starter feature: “Study material covering all 71 learning outcomes as per PMQ exam handbook” (`plans.ts`).
- **2026-09-21** — PMQ Pro Bundle: teal “Best Value” badge (`plans.ts`, `PmqPlanCards`).
- **2026-09-21** — PMQ Starter card: “Introductory Offer” badge beside the name (`plans.ts`, `PmqPlanCards`).
- **2026-09-21** — PMQ Pro/AI Pro price note: “one-off · No subscription” (capital N) (`plans.ts`).
- **2026-09-21** — Plan-card memory/takeaways icon → lucide Brain (same as LO Apply memory aids) (`PmqPreviewFeatureIcons`).
- **2026-09-21** — PFQ Pro CTA: “Get Pro Bundle” (`plans.ts`).
- **2026-09-21** — PFQ Starter takeaways/glossary line uses memory-doc icon (not misconceptions ✕) (`plans.ts`, `PfqPlanCards`).
- **2026-09-21** — PFQ Starter: capital S on “Study material…” feature line (`plans.ts`).
- **2026-09-21** — PFQ Pro/AI Pro “Insights…” line uses lightbulb `IconInsights` (same as LO Insights chip), not core stack (`PmqPreviewFeatureIcons`, `plans.ts`, `PfqPlanCards`).
- **2026-09-21** — PFQ Starter “Exam structure…” uses new pie `IconStructure` (not report clipboard) (`PmqPreviewFeatureIcons`, `plans.ts`, `PfqPlanCards`).
- **2026-09-21** — PFQ Starter “Exam structure…” line uses report icon (not mock monitor) (`plans.ts`).
- **2026-09-21** — PFQ Starter: capital E on “Exam structure…” feature line (`plans.ts`).
- **2026-09-21** — Plan-card feature labels: “End-of-course…” capital E; drop `text-wrap: pretty` so lines use full row width (`plans.ts`, `PmqPlanCards.module.css`).
- **2026-09-21** — PFQ AI Pro: inherits Starter (lists Pro unlocks + Sly + report) (`plans.ts`).
- **2026-09-21** — PFQ Pro features: 515 additional practice / 3 timed mocks / insights on every learning outcome (`plans.ts`).
- **2026-09-21** — PFQ Starter features: handbook outcomes line + “what actually matters” exam-structure line (`plans.ts`).
- **2026-09-21** — PFQ Starter feature lines rewritten (study material / takeaways glossary / 50 practice / exam structure) (`plans.ts`).
- **2026-09-21** — PFQ Starter price note: drop trailing full stop (`plans.ts`).
- **2026-09-21** — PFQ Starter price note: “No credit card needed” (`plans.ts`).
...
- **2026-09-21** — **PMQ repricing shipped in code** (Cursor): `registry-data` `priceCents` 800→1500; AI Pro indicative `plans.ts` 1500→2500; migration `20260921160000_pmq_pro_price_1500.sql` applied (`exam_config.ai_tutor_price_cents` 999→1500). Stripe still uses `price_data.unit_amount` (no fixed Price). Test-mode session observed `amount_total: 1500`. PFQ untouched.
- **2026-09-21** — **PMQ repricing, decided by Sim.** PMQ Pro Bundle **£8 → £15**; PMQ AI Pro Bundle **£15 → £25** (still `status: "waitlist"`, indicative, no checkout). PFQ is unaffected (Pro £10, AI Pro £20). Canonical value is `COURSE_STATIC["pmq-in-5-days"].priceCents` in `src/lib/courses/registry-data.ts` (800 → 1500), which `SLY_UNLOCK_PRICE_CENTS` re-exports and checkout charges; PMQ AI Pro card price is the one hardcoded figure, `priceCents: 1500` → `2500` in `src/lib/pmq/plans.ts`. Supabase `exam_config.ai_tutor_price_cents` for `pmq-in-5-days` still holds the stale 999 and needs a migration to 1500 so the DB stops disagreeing with the registry. Docs swept the same day (BUSINESS_STATE, PMQ_PRO_BUNDLE_SPEC_SHEET, PHILOSOPHY_PAGE_COPY + _DETAILED, FEATURES, REAL_MOCK_EXAM_SPEC, AI_TUTOR_BACKEND_SPEC, LAUNCH_RUNBOOK, PRODUCT, docs/roadmap, legal/PRIVACY_POLICY, legal/PRE_LAUNCH_CHECKLIST). Code, tests and the Supabase migration are Cursor's job.
- **2026-09-21** — PMQ Starter plan price note: “No credit card needed” (`plans.ts`).
- **2026-09-21** — Pricing plan cards: course kicker on each card (“PMQ in 5 days” / “PFQ in 2 days”, orange on days) — not a section heading (`PmqPlanCards`, `PfqPlanCards`).
- **2026-09-21** — Pricing pages: course label above plan cards (“PMQ in 5 days” / “PFQ in 2 days”, orange on days) so the two three-tier grids aren’t confused (`pmq`/`pfq` pricing pages).
- **2026-09-21** — PFQ pricing page: match PMQ heading (“Start free. / Upgrade only if you love it.” + crumb) and footer legal line; plan cards unchanged (`pfq-in-2-days/pricing/page.tsx`).
- **2026-09-21** — PMQ pricing page: remove header subcopy under “Start free.” (`pmq-in-5-days/pricing/page.tsx`).
...
- **2026-09-21** — Catalogue card subheads: real width fix — `text-wrap: pretty` was shortening centered lines (looked like a max-width); use normal wrap + `justify-self: stretch` (`CoursesCatalog.module.css`).
- **2026-09-21** — Catalogue card subheads: drop `max-width: 34ch`/`38ch` so copy uses full card width on mobile (`CoursesCatalog.module.css`).
- **2026-09-21** — Catalogue PMQ subhead: plain weight again (dropped bold/orange accents) (`CoursesCatalog`).
- **2026-09-21** — Catalogue PFQ card subhead: “All 10 learning objectives and 59 learning outcomes…” (`CoursesCatalog`, `courses-catalog`).
- **2026-09-21** — Catalogue PMQ card subhead: coverage sell (“24 learning objectives and 71 learning outcomes…”) replaces generic pass line (`CoursesCatalog`, `courses-catalog`).
- **2026-09-21** — Catalogue “View plans”: no hover wash / ink darken — underline only (`CoursesCatalog` `CARD_TERTIARY`).
- **2026-09-21** — Footer nav spacing: equal spread only on narrow phones (`justify-between`); from `sm` cluster right with gap; `lg` stays end-aligned — stops iPad from stretching links edge-to-edge (`SiteFooter`).
- **2026-09-21** — Catalogue PFQ card matches PMQ: Start free course → preview, Course overview → `/courses/pfq-in-2-days`, View plans → pricing (`CoursesCatalog`).
- **2026-09-21** — Course FAQ accordion: drop redundant “Frequently asked questions” subtitle under FAQs (`PmqFaqSection`, `PfqOverviewSections`).
- **2026-09-21** — Header Sign in / Menu: kill iOS sticky hover wash — fine-pointer-only hover + tap-highlight off + blur on Sign in click (`header-control`, `SiteHeaderControls`).
- **2026-09-21** — Header menu current page: orange text/icon only (no ink wash) (`SiteHeaderMenu`).
- **2026-09-21** — Method recall plate: opaque `#f2e8d5` (matches `memory.webp` bake). Prior `bg-cream/90` let teal bleed through behind contain-fit art, reading as a halo (`LabArtPlate`, `TestingMethod`).
