...
- **2026-09-21** — PMQ Starter recall activities: Pair up / Group up / Line up icons always visible on Learn; LO1 playable for all tiers; LO2–24 locked with padlock beside each icon (`canAccessRecallActivities(tier, loNumber)`, `redactPmqInsights` strips payloads, `ActivityLauncher` locked chrome via `StudyTable` / `CoreContentBlock` / `LoLearnStage`).
