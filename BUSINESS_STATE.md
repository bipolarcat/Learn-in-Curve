...
- **2026-09-15** — Home hero Impeccable audit fix-all (except video preload): visible-default motion (no blur/bounce/gated opacity), Framer “curve.” replaces GSAP `BouncingText`, eyebrow wraps at 11px mobile, canvas key two-pass + DPR cap 1.25, CTA pending links use `tabIndex={-1}`; removed `bouncing-text`, `gsap`/`@gsap/react`, unused `hero-animals-transparent.webp`; `DESIGN.md` hero section updated (`HomeBrandHero`, `HeroAnimalsScene`, `FreeMockExamLink`, `ExploreCoursesLink`).
