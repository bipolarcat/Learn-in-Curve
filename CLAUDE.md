...
- `cursor-prompt-*.md` files remain how Claude hands Cursor a specific piece of execution, but the authoritative record of what's outstanding and its priority is the Linear issue, not the prompt file. **All prompt files live in `cursor-prompts/` (moved off the project root 2026-09-15 to keep the root readable).** Active prompts sit at `cursor-prompts/`; finished ones move down into `cursor-prompts/archive/`. Linear tickets are what persist.

## Collaborators & tools

- **Cursor** = the external AI coding agent Sim runs locally (a different product from Claude/this session). Division of labor: Claude plans, writes specs, grooms the Linear backlog (team key `LIC`), verifies shipped work, and handles legal/content/docs; Cursor executes code changes in the actual repo. Claude hands Cursor work by writing a `cursor-prompt-*.md` file in `cursor-prompts/` (NOT the project root: they were moved there 2026-09-15). Check that folder to see what's queued or in flight for Cursor right now; anything in `cursor-prompts/archive/` is done. Cursor reports its own status back (either via a `BUSINESS_STATE.md` decision-log entry it appends itself, or pasted into chat by Sim) — **always verify a "done" claim against actual repo state (git log, file contents, DB row counts) before trusting it or moving a Linear ticket past In Review.** This project has already had cases of a "done" report that wasn't accurate once checked.
...
- **Missing inputs: stop and ask (standing rule, set 2026-09-21).** If Sim says a file, doc, report or input exists and it cannot be found (not in the repo, not attached, not at the path given), STOP and ask him where it is before doing any of the work. Do not proceed with the partial set and deliver an answer that then has to be reworked once the missing piece arrives. This is a token-cost rule as much as a quality one: redoing an analysis costs more than one question.
